...
function replicated_upgrade() {
  ...
  info ""
  bold " Non-fastforward Updates"
  info ""
  info " The 2.11 version of Gerrit requires that for the inline-edit functionality, Non-fastforward updates"
  info " be applied to internal \"edit\" refs. As previous versions of Git Multisite did not support"
  ...
  if [ "$scan_repos" == "true" ]; then
    if ! scan_repos; then
      info ""
      info " \033[1mWARNING:\033[0m Configuration updates incomplete. Manual configuration changes may"
      info " be required to use the in-line-edit Gerrit functionality on all repositories."
      info ""
    fi
  fi

  info ""
  bold " GitMS Gerrit Event Plugin"
  info ""
  info " Previous versions of GerritMS used a custom plugin, \"gitms-gerrit-event-plugin\" to pass reindex information"
  info " from Gerrit to the GitMS Replicator. This plugin however is no longer required, and must be deleted. "
  info ""

  if [ ! "$NON_INTERACTIVE" == "1" ]; then
    REMOVE_PLUGIN=$(get_boolean "Do you want to delete this plugin now?" "true")
  fi

  if [ "$REMOVE_PLUGIN" == "true" ]; then
    remove_gitms_gerrit_plugin
  fi
}
...
GERRIT_RELEASE_NOTES="https://gerrit-documentation.storage.googleapis.com/ReleaseNotes/ReleaseNotes-2.11.html"
...
PREVIOUS_ALLOWED_RP_GERRIT_VERSIONS=("v2.10.6-RP-1.6.0.1" "v2.10.6-RP-1.6.0.2" "v2.11.5-RP-1.7.0.1" "v2.11.6-RP-1.7.1.1" "v2.11.7-RP-1.7.1.4")
