...
function scan_repos() {
  ...
  export GIT_CONFIG
  gerritGitBase=$(git config --get gerrit.basepath || true)
  gerritGitBase=${gerritGitBase:-git}

  if [[ "$gerritGitBase" != /* ]]
  then
     #Assumes relative path if gerritGitBase doesn't start with '/'
     gerritGitBase="$GERRIT_ROOT/$gerritGitBase"
  fi

  [ ! -e "$gerritGitBase" ] && die "$gerritGitBase does not exist"
  [ ! -d "$gerritGitBase" ] && die "$gerritGitBase is not a directory"

  echo ""
  echo "Scanning for repositories in $gerritGitBase"
  echo ""

  find "$gerritGitBase" -type d -name '*.git' | while read repoPath; do
    add_repo "$repoPath"
  done
  return 0
}
